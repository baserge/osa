...
"""
from . import xmlnamespace, xmlparser, xmltypes
...
class XMLSchemaParser(object):
    ...
    def __init__(self, root, wsdl_url=None):
        ...
        if root.tag != "{%s}schema" % xmlnamespace.NS_XSD:
            raise ValueError(
                "Supplied root node '%s' is not of XML Schema " "type." % root.tag
            )
        ...
        for schema in imports:
            ...
            if loc in (
                xmlnamespace.NS_SOAP,
                xmlnamespace.NS_SOAP12,
                xmlnamespace.NS_SOAP_ENC,
                xmlnamespace.NS_SOAP_ENV,
                xmlnamespace.NS_WSDL,
                xmlnamespace.NS_XSD,
                xmlnamespace.NS_XSI,
            ):
                continue
            ...
            parser = XMLSchemaParser(
                xmlparser.parse_qualified_from_url(loc, wsdl_url=wsdl_url),
                wsdl_url=wsdl_url,
            )
    ...
    def create_type(name, element, xtypes, types):
        ...
        if element.tag == "{%s}element" % xmlnamespace.NS_XSD:
            ...
            if len(children) == 1:
                ...
                raise ValueError(
                    " Wrong schema structure. Element has more than 1 "
        ...
        if element.tag == "{%s}complexType" % xmlnamespace.NS_XSD:
            ...
            element.tag == "{%s}simpleType" % xmlnamespace.NS_XSD
            and element[0].tag == "{%s}restriction" % xmlnamespace.NS_XSD
    ...
    def create_alias(name, alias_type, xtypes, types):
        ...
        cls = type(
            cls_name, (alias,), {"__doc__": "no documentation", "_namespace": cls_ns}
        )
        types[name] = cls
    ...
    def create_empty_class(name, types):
        ...
        cls_name = xmlnamespace.get_local_name(name)
        cls_ns = xmlnamespace.get_ns(name)
        cls = xmltypes.ComplexTypeMeta(
            cls_name,
            [],
            {"_children": [], "__doc__": "no documentation", "_namespace": cls_ns},
        )
        types[name] = cls
    ...
    def create_string_enumeration(name, element, types):
        ...
        cls = type(
            cls_name,
            (xmltypes.XMLStringEnumeration,),
            {"_allowedValues": values, "__doc__": doc, "_namespace": cls_ns},
        )
        types[name] = cls
    ...
    def create_complex_class(name, element, xtypes, types):
        ...
        parents = []
        exts = element.findall(
            "./{%s}complexContent/{%s}extension"
            % (xmlnamespace.NS_XSD, xmlnamespace.NS_XSD)
        )
        if exts is not None:
            for ext in exts:
        ...
        cls = xmltypes.ComplexTypeMeta(
            cls_name,
            parents,
            {"_children": children, "__doc__": doc, "_namespace": cls_ns},
        )
        types[name] = cls
